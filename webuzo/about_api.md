...
## via CuRL
```php
curl -d "softsubmit=1" -d "softdomain=example.com" -d "softdirectory=wp" -d "softdb=wpdb" -d "admin_username=admin" -d "admin_pass=adminpassword" -d "admin_email=admin@example.com" -d "language=en" -d "site_name=Wordpress Site" -d "site_desc=My Blog" -d "dbprefix=dbpref_" -d "sets_name[]=set-name" "https://user:password@domain.com:2003/softaculous/index.php?act=software&soft=26&api=json"
```
...
$url = 'https://user:password@domain.com:2003/softaculous/index.php?api=serialize&act=software&soft=26';
